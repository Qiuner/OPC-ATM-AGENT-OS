...
function ThumbUpIcon() {
  return (
    <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="#b0b3b8" strokeWidth={1.5}>
...
export function MetaPreview({ data }: MetaPreviewProps) {
  const tagsText = data.tags.map((t) => `#${t}`).join(' ');

  return (
    <BrowserFrame url="facebook.com">
      <div style={{ background: '#242526', minHeight: '400px', padding: '12px' }}>
        {/* Post card */}
        <div className="rounded-lg overflow-hidden" style={{ background: '#242526' }}>
          ...
          <div className="flex items-center justify-between px-4 py-2">
            <div className="flex items-center gap-1">
              ...
              <div className="flex -space-x-1">
                <div className="h-[18px] w-[18px] rounded-full flex items-center justify-center text-[10px]" style={{ background: '#1877f2', zIndex: 3 }}>
                  👍
                </div>
                <div className="h-[18px] w-[18px] rounded-full flex items-center justify-center text-[10px]" style={{ background: '#f33e58', zIndex: 2 }}>
                  ❤️
                </div>
                <div className="h-[18px] w-[18px] rounded-full flex items-center justify-center text-[10px]" style={{ background: '#f7b928', zIndex: 1 }}>
                  😆
                </div>
              </div>
              <span className="text-[13px] ml-1" style={{ color: '#b0b3b8' }}>
                You and 128 others
              </span>
            </div>
            <span className="text-[13px]" style={{ color: '#b0b3b8' }}>
          ...
          >
            {[
              ...
              <button
                key={label}
                className="flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-[15px] font-medium transition-colors"
                style={{ color: '#b0b3b8' }}
              >
                <Icon />
                {label}
