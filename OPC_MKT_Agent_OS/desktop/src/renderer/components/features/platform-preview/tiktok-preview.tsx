...
export function TikTokPreview({ item }: PlatformPreviewProps) {
  ...
  return (
    <PhoneFrame>
      <div className="relative" style={{ background: '#000', minHeight: '500px' }}>
        {/* Top tabs */}
        ...
        >
          {/* Play button */}
          <div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-14 w-14 flex items-center justify-center rounded-full"
            style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)' }}
          >
            <svg className="h-7 w-7 ml-1" viewBox="0 0 24 24" fill="white">
              <polygon points="5 3 19 12 5 21 5 3" />
            </svg>
          </div>

          {/* Right interaction bar */}
          <div className="absolute right-2 bottom-[80px] flex flex-col items-center gap-4">
            {/* Avatar + follow */}
            <div className="relative">
              ...
              <div
                className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 h-4 w-4 rounded-full flex items-center justify-center text-[12px] font-bold text-white"
            ...
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-0.5">
                <svg className="h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={1.5}>
                  <path d={btn.path} />
                </svg>
            ...
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #333, #000)', border: '2px solid #333' }}
            />
          </div>

          {/* Bottom text overlay */}
          <div className="absolute left-3 bottom-[12px] right-[60px]">
            <span className="text-[14px] font-semibold text-white">
              {handle}
            </span>
            <p className="text-[13px] text-white mt-1 leading-[17px] line-clamp-2">
              {body.slice(0, 100)}
              {tags.length > 0 && ' '}
              {tags.map((t) => (
        ...
        {/* Bottom nav */}
        <div className="flex items-center justify-around py-2 mt-1" style={{ background: '#000' }}>
          {['Home', 'Friends', '', 'Inbox', 'Profile'].map((label, i) => (
            <div key={i} className="flex flex-col items-center">
              {i === 2 ? (
                <div
                  className="h-7 w-10 rounded-lg flex items-center justify-center"
                ...
                <>
                  ...
                  />
                  <span className="text-[9px] mt-0.5" style={{ color: i === 0 ? '#fff' : 'rgba(255,255,255,0.5)' }}>
                    {label}
                  </span>
                </>
              )}
