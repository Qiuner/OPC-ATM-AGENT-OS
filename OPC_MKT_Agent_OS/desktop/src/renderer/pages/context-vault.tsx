...
export function ContextVaultPage(): React.JSX.Element {
  ...
  const [versionDialogAsset, setVersionDialogAsset] =
    useState<ContextAssetWithVersions | null>(null);
  ...
  // Load existing product cards from assets
  useEffect(() => {
    const products = assets
      .filter(
        (a) =>
          a.type === "product" &&
          (a.metadata as Record<string, unknown>)?.source_url
      )
      .map((a) => {
  ...
  const handleDeleteProduct = useCallback(
    async (productId: string) => {
      try {
        const api = getApi();
        if (!api) return;
        const res = await api.context.delete(productId);
        if (res.success) {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      const json = res as {
        success: boolean;
        data?: {
          email?: { address: string; senderName: string; verified: boolean };
        };
      };
    ...
    try {
      ...
      const res = await api.settings.update({
        email: {
          ...
          senderName:
            senderName.trim() || emailAddress.trim().split("@")[0],
          verified: false,
  ...
  const fetchAssets = useCallback(async () => {
    try {
      ...
      const api = getApi();
      if (!api) return;
      const res = await api.context.list();
      if (res.success && res.data) {
        setAssets(res.data as ContextAssetWithVersions[]);
      }
    } catch {
      // silently fail
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedAsset) {
        const res = await api.context.update(selectedAsset.id, {
          title,
          content,
          type,
          scope,
          ...
          setAssets((prev) =>
            prev.map((a) =>
              a.id === selectedAsset.id
                ? (res.data as ContextAssetWithVersions)
                : a
            )
          );
        }
  ...
  // ---------- Helpers ----------
  const getStatus = (
    asset: ContextAssetWithVersions
  ): "complete" | "incomplete" => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground">Context Vault</h1>
        ...
        {brandSetupOpen && (
          <div className="mt-5 space-y-5">
              ...
              <div className="space-y-5">
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: "var(--muted-foreground)" }}
                  >
                    Product URL scraping requires the Agent engine. Please
                    manually add product assets below for now.
                  </p>
                </div>
                ...
                {productCards.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p
                      ...
                      style={{ color: "var(--muted-foreground)" }}
                    >
                      No product cards yet. Add products via &quot;新增资产&quot;
                      button.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {productCards.map((product) => (
                      <div
                        key={product.id}
                        className="rounded-xl p-4 dark:bg-white/[0.03] bg-black/[0.02] transition-all hover:-translate-y-0.5"
                        style={{
            ...
            {brandTab === "skills" && (
              <div className="space-y-5">
                <label
                  className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-xl cursor-pointer hover:border-[#a78bfa] dark:hover:bg-white/[0.02] hover:bg-black/[0.02] transition-colors"
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: "var(--muted-foreground)" }}
                  >
                    Custom Skills management coming soon
                  </span>
                  <span
                    className="text-xs mt-1"
                    style={{ color: "var(--muted-foreground)" }}
                  >
                    Skills API will be available in a future update
                  </span>
                </label>
              </div>
            )}
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: "var(--muted-foreground)" }}
                    >
                      Email sending will be available in Phase 2. Configure
                      your address now so the Email Marketing Agent knows
        ...
        >
          {filteredAssets.length === 0 ? (
            ...
            filteredAssets.map((asset, idx) => {
              ...
              const needsTruncate =
                asset.content.length > CONTENT_TRUNCATE_LENGTH;
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      {imgs.length > 0 ? (
                        ...
                        >
                          <Icon
                            className={`size-4 ${config.textClass}`}
                          />
                        </div>
                      )}
      ...
      >
        <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              历史版本 — {versionDialogAsset?.title}
            </DialogTitle>
            <DialogDescription>
              共{" "}
              {(
                versionDialogAsset?.metadata
                  ?.versions as ContextAssetVersion[] | undefined
              )?.length ?? 0}{" "}
              个历史版本
            </DialogDescription>
          </DialogHeader>
            ...
            {(
              ...
              .map((ver, idx) => (
                ...
                >
                  ...
                  <Badge
                    variant="secondary"
                    className={typeConfig[ver.type].badgeClass}
                  >
                    {typeLabels[ver.type]}
                  </Badge>
                  <Badge
              ...
              {images.length < MAX_IMAGES && (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: "var(--muted-foreground)" }}
                  >
                    支持 JPG/PNG/WebP，单张 &le; 2MB，最多 {MAX_IMAGES}{" "}
                    张
                  </span>
                  <input
                    ref={fileInputRef}
